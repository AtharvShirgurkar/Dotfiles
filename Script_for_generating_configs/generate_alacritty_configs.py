...
# 1. Output directory
OUTPUT_DIR = "alacritty_configs"
if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

# 2. The Official Catppuccin Palette (Hex Codes)
COLORS = {
    "latte": {
    ...
    "macchiato": {
        ...
        "subtext0": "#a6da95", "overlay2": "#9399b2", "overlay1": "#8087a2", "overlay0": "#6e738d",
...
# 3. The Template
# We inject the {accent} color into cursor and selection.
# Standard colors (red, green, etc) remain mapped to their specific palette definitions.
TEMPLATE = """
[colors.primary]
background = "{base}"
foreground = "{text}"
dim_foreground = "{overlay1}"
...
def generate_configs():
    ...
    for flavor, palette in COLORS.items():
        for accent_name in ACCENTS:
            accent_hex = palette[accent_name]
            
            # Format the template using the current flavor's palette
            # and the current loop's accent color
            config_content = TEMPLATE.format(
                accent_hex=accent_hex,
                **palette # Unpack the rest of the palette (base, text, red, etc.)
            )
            
            # Construct filename: catppuccin-frappe-blue.conf
            filename = f"catppuccin-{flavor}-{accent_name}.toml"
    ...
    print(f"Successfully generated {count} configuration files.")
